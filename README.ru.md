...
| **Базы данных** | Все базы MySQL и SQLite: размер, таблицы, строки, индексы, соединения, рост за неделю |
| **Карточка базы** | Из чего сложился размер, топ-20 таблиц, активные соединения, история размера |
...
* MySQL/MariaDB и/или SQLite — необязательно, для раздела баз

PostgreSQL пока не поддержан: панель писалась под MySQL и SQLite, а выкладывать
непроверенный на живых данных код означало бы нарушить то самое правило про
выдуманные цифры. Слой баз — один модуль `lib/dbs.py`, если захочется добавить.
...
* **Базы** — `information_schema` для MySQL (через системный клиент и
  `/etc/mysql/debian.cnf`, поэтому пароль в панели не хранится), `PRAGMA` и
  `dbstat` для SQLite, открытой только на чтение в режиме immutable, чтобы не
  помешать пишущему сайту.
